...
df <- read.csv("E:/ZEZO/fourth year/Second Term/Distributed Computing/project/data_science/OnlyNumeric_Data.csv")
...
selected_cols <- colnames(corr)[abs(corr["SalePrice", ]) > 0.22]
...
model <- train(SalePrice ~ ., data = train , method = "svmRadial", trControl = trainControl(method = "cv", number = 15), preProcess = c("center", "scale"))
warnings()
...
cv_results <- train(SalePrice~., data = data, method = "svmRadial", trControl = trainControl(method="cv", number=15), preProcess = c("center", "scale"))
print(cv_results)